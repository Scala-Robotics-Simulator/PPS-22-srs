...
- La gestione del salvataggio e caricamento del file di configurazione ha richiesto più tempo del previsto, a causa della necessità di gestire la serializzazione e deserializzazione degli oggetti complessi in maniera personalizzata.
- Un errore nella modellazione del dominio, relativo alle azioni che il robot può svolgere, ha reso necessaria una seconda revisione della modellazione delle azioni stesse oltre al comportamento dei robot, causando ritardi rispetto alla pianificazione iniziale.
- Il ritardi descritti sopra hanno impattato negativamente sulla pianificazione dello sprint, rendendo necessario il rinvio dell'implementazione dei sensori di luce al prossimo sprint.
