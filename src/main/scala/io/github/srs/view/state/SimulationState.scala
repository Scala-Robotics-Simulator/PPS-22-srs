...
final case class SimulationState(
  ...
  def withEnvironment(env: Environment): SimulationState =
    copy(
      environment = Some(env),
      robots = env.robots.sortBy(_.id),
    )

  def withSelection(id: Option[String]): SimulationState =
    copy(selectedRobotId = id)

  def withStaticLayer(img: BufferedImage, size: (Int, Int, Int, Int)): SimulationState =
