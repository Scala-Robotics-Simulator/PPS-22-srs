  ...
  it should "only contain boundaries if no entities are passed" in:
    inside(Environment(10, 10).validate):
      case Right(environment) =>
        environment.entities.map(b => (b.position, b.orientation, b.shape)) should contain theSameElementsAs
          createBoundaries(10, 10).map(b => (b.position, b.orientation, b.shape))
