...
/** Tests for [[IlluminationLogic]]: statics prep, reuse, and FoV wiring. */
...
//      val wall = StaticEntity.Obstacle(pos = WallPos, orient = Orientation(0), width = WallW, height = WallH)
//      val bot =
//        (robot at RobotPos withShape ShapeType.Circle(RobotR) withOrientation Orientation(0)).validate.toOption.value
//      val light = StaticEntity.Light(pos = LightPos, orient = Orientation(0), illuminationRadius = LightR, intensity = LightI)
...
//    val prepared = Illumination.prepareStatics(env, summon[ScaleFactor])
//
//    val biggerWall = StaticEntity.Obstacle(pos = C.WallPos, orient = Orientation(0), width = 1.2, height = C.WallH)
