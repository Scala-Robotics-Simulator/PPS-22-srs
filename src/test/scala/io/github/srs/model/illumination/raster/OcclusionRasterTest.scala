...
final class OcclusionRasterTest extends AnyFlatSpec with Matchers:
  ...
  private object C:
    ...
    def validated(env: Environment): Environment =
      env.validate(insertBoundaries = false).toOption.value
    ...
    val AnglesSome: Seq[Double] =
      List(1.0, 15.0, 30.0, 45.0, 60.0, 75.0, 89.0, 123.0, 179.0, 225.0, 271.0, 315.0, 359.0)
    val AnglesOther: Seq[Double] = List(0.0, 15.0, 45.0, 60.0, 89.0, 123.0, 179.0, 225.0, 315.0, 359.0)
  end C
  ...
  "OcclusionGrid (Circle)" should "block when radius fully covers the cell and be free otherwise" in:
    ...
    val botAbove = Robot((0.5, 0.5), ShapeType.Circle(tie + eps), Orientation(0), Vector.empty)
  ...
  "OcclusionGrid (Rectangle)" should "be opaque at the 1x1 tie and free only when strictly smaller" in:
    ...
    val obsIn = StaticEntity.Obstacle(Point2D(0.5, 0.5), Orientation(0), width = 1.001, height = 1.001)
  ...
  "OcclusionGrid (Rectangle rotation)" should "block when fully covering the cell and be free otherwise" in:
    ...
    val small = StaticEntity.Obstacle(Point2D(0.5, 0.5), Orientation(45.0), width = 0.8, height = 0.8)
  ...
  "Rectangle 1x1 at (0,0) — orthogonal and non-orthogonal" should "always be transparent" in:
    val allTransparent: Boolean = C.AnglesOther.forall { ang =>
      val obs = StaticEntity.Obstacle(Point2D(0.0, 0.0), Orientation(ang), width = 1.0, height = 1.0)
      val env = C.validated(environment withWidth 1 withHeight 1 containing obs)
      val stat = OcclusionRaster.staticMatrix(env)
      stat(0)(0) == 0.0
  ...
  "Rectangle 1x1 at (0,0) — any orientation" should "be transparent" in:
    val angles = C.AnglesOther
    val allTransparent: Boolean = angles.forall { ang =>
      val obs = StaticEntity.Obstacle(Point2D(0.0, 0.0), Orientation(ang), width = 1.0, height = 1.0)
      val env = C.validated(environment withWidth 1 withHeight 1 containing obs)
      val stat = OcclusionRaster.staticMatrix(env)
      stat(0)(0) == 0.0
  ...
  "OcclusionGrid (Rectangle 1x1 centered) — tie case" should "block when the 4 corners lie on the cell border" in:
    ...
    val obsBelow = StaticEntity.Obstacle(Point2D(0.5, 0.5), Orientation(0), width = 0.999, height = 0.999)
    ...
    val obsAbove = StaticEntity.Obstacle(Point2D(0.5, 0.5), Orientation(0), width = 1.001, height = 1.001)
