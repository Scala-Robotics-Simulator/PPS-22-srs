...
//    val wall = StaticEntity.Obstacle(pos = C.WallPos, orient = C.Orient0, width = C.WallW, height = C.WallH)
//    val bot =
//      (robot at C.RobotPos withShape ShapeType.Circle(C.RobotR) withOrientation C.Orient0).validate.toOption.value
//    val light = StaticEntity.Light(pos = C.LightPos, orient = C.Orient0, illuminationRadius = C.LightR, intensity = C.LightI)
...
//    val biggerWall = StaticEntity.Obstacle(pos = C.WallPos, orient = C.Orient0, width = C.WallW + 0.2, height = C.WallH)
